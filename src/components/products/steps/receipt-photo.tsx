...
export function ReceiptPhoto({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!res.ok) {
        setError({ notConfigured: false, message: body.message ?? "Couldn't read that receipt." });
        return;
      }

      const data: ReceiptExtraction = body.data;
      const partial: Partial<ProductDraft> = {};
      const aiFilled: (keyof ProductDraft)[] = [];

      for (const [apiKey, draftKey] of Object.entries(FIELD_MAP)) {
        const value = data[apiKey as keyof ReceiptExtraction];
      ...
      if (typeof data.price === "number") {
        partial.purchasePrice = String(data.price);
        aiFilled.push("purchasePrice");
      }
      if (typeof data.purchase_date === "string" && data.purchase_date) {
        partial.purchaseDate = data.purchase_date;
        aiFilled.push("purchaseDate");
      }

      const uncertain = (data.uncertain ?? [])
        .map((k) => (k === "price" ? "purchasePrice" : k === "purchase_date" ? "purchaseDate" : FIELD_MAP[k]))
  ...
  return (
    <div className="p-4">
      <PhotoCaptureArea
        hint={"Photograph your paper receipt\nor screenshot a digital one"}
      ...
      {loading ? (
        ...
        <div className="mt-3.5 flex items-center gap-1.5 rounded-[10px] bg-teal/10 p-3 text-xs leading-snug text-teal">
          ...
          Buddy reads product name, price, purchase date, and retailer from your receipt
        </div>
      )}
    </div>
  );
